...
## Overview
Analyze OpenAPI specification diffs generated by `oasdiff` and generate professional changelogs following Mintlify documentation standards with versioned changelog files.

## Input Format
The input is the output from `oasdiff diff`, which provides a structured comparison of two OpenAPI specifications. The format includes:
...
### Deleted Endpoints: N
------------------------
METHOD /path/to/removed/endpoint
```
...
--------------------------
METHOD /path/to/endpoint
- New query param: param_name
- Deleted query param: old_param
- Modified query param: param_name
  - Schema changed
  ...
  - Modified response: 200
    - Content changed
      - Modified media type: application/json
        - Schema changed
          - Properties changed
            - New property: new_field
            - Deleted property: old_field
            - Modified property: field_name
...
### 1. Server URLs
Ignore any changes related to the `servers` section - these are environment-specific URLs (e.g., localhost vs. production).
...
Ignore removal of `cursor` and `size` parameters from individual endpoints. The OpenAPI spec generation process removes duplicate parameter definitions when they're inherited globally. These parameters remain available through global definitions - functionality is unchanged.
...
### 5. Cosmetic Schema Changes
Ignore minor cosmetic changes that don't affect API behavior:
- Title changes (e.g., `Title changed from 'ModelName' to 'ModelNameRead'`)
- Description wording improvements that don't change semantics
- Extension changes that only add `examples`
- Example value changes (e.g., `Example changed from '2026-02-10T...' to null`)

## Task
1. Analyze the oasdiff output for: new endpoints, deleted endpoints, modified endpoints (parameter changes, response changes, schema modifications)
...
4. Detect breaking changes vs. improvements based on the change types
...
Lists all newly added endpoints. Each line shows `METHOD /path`:
```
### New Endpoints: 11
---------------------
GET /v2/telematics/analytics/drivers
POST /v2/telematics/analytics/create
```
**Action**: Document as new features/additions.

### Deleted Endpoints Section
Lists all removed endpoints:
```
### Deleted Endpoints: 2
------------------------
GET /v2/telematics/graphql
POST /v2/telematics/graphql
```
**Action**: Document as breaking changes requiring migration.

### Modified Endpoints Section
Shows detailed changes for each modified endpoint:
- **New query param**: A new optional parameter was added
- **Deleted query param**: A parameter was removed (breaking change)
- **Modified query param**: Parameter definition changed
- **New property**: Response body has new field
- **Deleted property**: Response body field removed (breaking change)
- **Modified property**: Field definition changed
- **Type changed**: Data type modification (often breaking)
- **Format changed**: Field format modification
- **Description changed**: Documentation update only

**Breaking changes to highlight**:
- Deleted query params
- Deleted properties in responses
- Type changes (e.g., `string` to `integer`)
- Required field additions
- Renamed endpoints/parameters
